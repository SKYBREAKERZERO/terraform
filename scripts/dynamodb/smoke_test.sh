...
TEST_FILE="${PROJECT_ROOT}/python/tests/dynamodb/test_dynamodb.py"
...
export DYNAMODB_TTL_ATTRIBUTE_NAME="${DYNAMODB_TTL_ATTRIBUTE_NAME:-expires_at}"
...
export DYNAMODB_TABLE_CLASS="${DYNAMODB_TABLE_CLASS:-STANDARD}"

export DYNAMODB_TEST_ITEM_JSON="${
  DYNAMODB_TEST_ITEM_JSON:-{\"id\":\"pytest-dynamodb-item-001\",\"source\":\"pytest\",\"status\":\"smoke-test\"}
}"
...
if [[ ! -f "${TEST_FILE}" ]]; then
  echo "[ERROR] DynamoDB test file was not found:"
  echo "  ${TEST_FILE}"
  exit 1
fi

echo "[PASS] test_dynamodb.py exists."
...
echo
echo "======================================================================"
echo "DYNAMODB SMOKE TEST RESULT"
echo "======================================================================"

if [[ ${PYTEST_EXIT_CODE} -eq 0 ]]; then

  echo
  echo "[PASS] DynamoDB smoke test completed successfully."
  echo

else

  echo
  echo "[FAIL] DynamoDB smoke test failed."
